...
class EnvOne(gym.Env):
    ...
    def __init__(self, size: int = 20, target_size: int = 3, alphabet=None,
    ...
    def _generate_grid(self):
        ...
        if self.target_size in COMMON_WORDS_BY_LENGTH:
            ...
            # not in common words
            target_start = self.np_random.integers(
                0, self.size - self.target_size + 1
            )
            target_string = ''.join(
                grid[target_start:target_start + self.target_size]
            )
            return (grid, target_string, target_start,
    ...
    def _get_obs(self):
        ...
        if 0 <= self._agent_location < self.size:
    ...
    def reset(self, **kwargs):
        ...
        (self._grid, self._target_string,
         self._target_start_pos, self._target_end_pos) = self._generate_grid()
